...
# ---- Fix script helpers -----------------------------------------------
# NOTE: .fixes_initialised and .modified_tables are reset at the START
# of each run_interactive_qc() call via reset_fixes_state(), NOT at source
# time, so that re-sourcing qc_helpers.R mid-session does not accidentally
# re-trigger the "keep or new?" prompt on the next appended fix.

.fixes_initialised <- FALSE
.modified_tables   <- character(0)

reset_fixes_state <- function() {
  .fixes_initialised <<- FALSE
  .modified_tables   <<- character(0)
}
...
append_fix <- function(snippet, mission_id, issue_type, tbl, col) {
  ...
  if (!.fixes_initialised) {
    ...
    if (start_fresh) {
      if (file.exists(.fixes_script)) file.remove(.fixes_script)
      ...
      # Write header only for a fresh script
      cat(
        "# ============================================================\n",
      ...
      cat(if (.lang == "f")
        "✅ Script existant conserve (mode ajout).\n"
      else
        "✅ Keeping existing script (append mode).\n")

      # In append mode, optionally add OPENING block if missing
      existing_keep <- if (file.exists(.fixes_script)) readLines(.fixes_script) else character(0)
      has_opening <- any(startsWith(trimws(existing_keep), "# OPENING"))

      if (!has_opening) {
        add_opening_keep <- prompt(
          if (.lang == "f")
            "  Le script existant n'a pas de bloc d'ouverture. L'ajouter maintenant ? [o]ui / [n]on > "
          else
            "  Existing script has no opening block. Add it now? [y]es / [n]o > ",
          if (.lang == "f") c("o", "n") else c("y", "n")
        )

        if (add_opening_keep %in% c("y", "o")) {
          opening_text <- .make_opening_block()
          opening_lines <- strsplit(opening_text, "\n", fixed = TRUE)[[1]]
          writeLines(c(opening_lines, "", existing_keep), .fixes_script)
          cat(if (.lang == "f")
            "✅ Bloc d'ouverture ajouté au script existant.\n"
          else
            "✅ Opening block added to existing script.\n")
        }
      }
    }
  ...
  if (done %in% c("y", "o")) {
    ...
    cat(if (.lang == "f")
      "✅ Bloc de sauvegarde ajouté. Le script est prêt à être exécuté.\n"
    else
